...
*IntoTheDeep* and *IntoTheDeepRR*, and uploaded them into your teams' GitHub accounts.
I have also pulled them down onto the PSA shared laptops (all teams).

If you already have GitHub credentials from a prior season (*CenterStage* or *PowerPlay*) on your personal laptop,
you should be able to clone either (or both) of the *IntoTheDeep* repositories without my help.
...
dir
```
You should see a list of your current repositories, which may include *CenterStage*, *CenterStageRR*, *CenterStageQQ*, *PowerPlay* or *PowerPlayRR*.
...
and not show you as being inside a `PowerPlay` or `CenterStage` repository.
...
delete the *PowerPlay* or *CenterStage* reference, and replace it with either *IntoTheDeep* or *IntoTheDeepRR*.
Leave the `.git` suffix attached.

(If it's not already clear:
Clone *IntoTheDeep* if you want the plain FTC SDK.
Clone *IntoTheDeepRR* if you want the Acme Robotics RoadRunner Quickstart SDK.
